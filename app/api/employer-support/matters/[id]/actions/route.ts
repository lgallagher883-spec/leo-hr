...
export async function PATCH(request:Request,{params}:Ctx){
 ...
 const body=await request.json().catch(()=>null) as {actionId?:unknown;status?:unknown}|null;const actionId=Number(body?.actionId);const status=body?.status==="done"?"done":body?.status==="open"?"open":null;if(!Number.isSafeInteger(actionId)||actionId<=0)return NextResponse.json({error:"Invalid action."},{status:400});if(!status)return NextResponse.json({error:"Invalid action status."},{status:400});
 const supabase=await createClient();const {data:{user},error:userError}=await supabase.auth.getUser();if(userError||!user)return NextResponse.json({error:"Please sign in again."},{status:401});const {data,error}=await (supabase as any).from("leo_employer_support_actions").update({status,completed_at:status==="done"?new Date().toISOString():null}).eq("id",actionId).eq("matter_id",matterId).eq("organisation_id",gate.access.organisationId).select("id,title,status").maybeSingle();
 if(error){console.error("Employer Support action update failed:",error);return NextResponse.json({error:"The action could not be updated."},{status:500});}
 if(!data)return NextResponse.json({error:"That action could not be found in this matter."},{status:404});

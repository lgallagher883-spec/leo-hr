...
export async function POST(request:Request,{params}:Ctx){
 ...
 const {data:current,error:currentError}=await (supabase as any).from("matters").select("status").eq("id",matterId).eq("organisation_id",gate.access.organisationId).eq("product_source","employer_support").maybeSingle();
 ...
 if((openActionCount??0)>0)return NextResponse.json({error:`Complete or remove the remaining ${openActionCount} open action${openActionCount===1?"":"s"} before closing this matter.`},{status:409});

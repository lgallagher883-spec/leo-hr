...
export async function GET(_request:Request,{params}:Ctx){
 ...
 const {data:document,error}=await (admin as any).from("matter_documents").select("id,matter_id,storage_path,file_name").eq("id",docId).eq("matter_id",matterId).maybeSingle();

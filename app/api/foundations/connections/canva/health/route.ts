...
import {
  getAuthenticatedCanvaConnection,
  getCanvaProfile,
  getCanvaUser,
} from "@/lib/canva/auth";
...
function getConnectionId(requestUrl: URL) {
  const value = requestUrl.searchParams.get("connectionId")?.trim() || "";
...
export async function GET(request: Request) {
  ...
  const access = await requireHealthAccess();

  if (!access.ok) {
    return access.response;
  }

  const { admin, organisationId, user } = access;
  ...
  if (!connectionId) {
    return NextResponse.json(
      {
        success: false,
        error: "A valid Canva connection ID is required.",
      },
      { status: 400 },
    );
  }
  ...
  const connectionResult = await admin
    ...
    .select("id, organisation_id, provider_id, status, account_display_name")
    .eq("id", connectionId)
  ...
  if (connectionResult.error || !connectionResult.data) {
    return NextResponse.json(
      { success: false, error: "The Canva connection could not be found." },
      { status: 404 },
    );
  }
  ...
  try {
    const authenticated = await getAuthenticatedCanvaConnection(connectionId);
    const [userResult, profileResult] = await Promise.all([
      getCanvaUser(authenticated.accessToken),
      getCanvaProfile(authenticated.accessToken),
    ]);

    const externalAccountId = userResult.team_user?.user_id || null;
    const externalWorkspaceId = userResult.team_user?.team_id || null;
    ...
    await admin.from("connection_health_checks").insert({
      ...
      status: "Healthy",
      summary: "Canva connection is healthy and authorised.",
      diagnostic_details: {
        external_account_id: externalAccountId,
        external_workspace_id: externalWorkspaceId,
        token_refreshed: authenticated.tokenRefreshed,
      },
    });
    ...
    await admin.from("connection_activity_history").insert({
      ...
      provider_id: connectionResult.data.provider_id,
      connection_id: connectionId,
      job_id: null,
      module_key: "Foundations",
      activity_type: "Connection Health Check",
      activity_summary: "Canva connection was verified successfully.",
      activity_details: {
        account_display_name: accountDisplayName,
        external_account_id: externalAccountId,
        external_workspace_id: externalWorkspaceId,
    ...
    return NextResponse.json({
      success: true,
      message: authenticated.tokenRefreshed
        ? "Canva token refreshed and connection verified."
        : "Canva connection verified.",
    ...
    const errorMessage =
      error instanceof Error
        ? error.message
        : "The Canva connection health check failed.";
    ...
    await admin
      .from("organisation_connections")
      .update({
        status: reconnectRequired ? "Reconnect Required" : "Needs Attention",
        health_status: reconnectRequired
          ? "Authentication Failed"
          : "Unavailable",
        last_failed_use_at: now,
        last_health_check_at: now,
        reconnect_required_at: reconnectRequired ? now : null,
        last_error_code: reconnectRequired
          ? "canva_reconnect_required"
          : "canva_health_check_failed",
    ...
    await admin.from("connection_health_checks").insert({
      connection_id: connectionId,
      check_type: "Connection",
      status: reconnectRequired ? "Authentication Failed" : "Unavailable",

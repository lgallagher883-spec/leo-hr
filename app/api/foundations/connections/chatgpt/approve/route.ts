...
export async function POST(request: Request) {
  try {
    ...
    const updateResult = await admin
      ...
      .update({
        ...
        connection_settings: {
          ...
          access_mode: "privacy_filtered_read_only",
        ...
        consent_record: {
          ...
          permitted_data: [
            ...
            "Company Knowledge",
            "Employee minimum operational identity: name and position/job title",
            "Workforce due-diligence metadata: check or requirement type, due/expiry date and operational status only",
            "Learning and qualification operational metadata: requirement type, due/expiry or completion status, and organisation-level counts",
            "Matter administrative shell: linked employee name where required, Matter type, status and created date only",
            "Talent onboarding aggregate workload and checklist status/due-date metadata without candidate personal data",
          ],
          prohibited_data: [
            "employee personal email, phone, home address, date of birth or National Insurance number",
            "employee banking, payroll or tax details",
            "medical information or disability/health records",
            "emergency contact details",
            "DBS certificate numbers, Update Service identifiers, passport numbers, driving licence numbers, right-to-work share/reference codes or other unique identity/document identifiers",
            "uploaded employee documents, evidence or document contents",
            "employee timeline or audit narrative",
            "Matter title, subject, description, allegations, messages, investigation notes, witness statements, meeting notes, evidence, documents, medical evidence or safeguarding details",
            "SAR request contents or disclosure material",
            "candidate names, email addresses, phone numbers, CVs, application narrative, interview notes, payroll details, due-diligence evidence or identity documents",
          ],
          prohibited_actions: [
            "calling Ask Leo",
            "sending emails or external communications",
            "employee record writes",
            "Matter writes, closure, deletion, disciplinary outcomes, dismissal, termination, redundancy selection or other consequential employment decisions",
            "candidate or onboarding record writes",
            "policy or compliance writes",
            "billing changes",
    ...
    await admin.from("connection_activity_history").insert({
      ...
      activity_summary: "ChatGPT privacy-filtered read-only business assistant access approved.",
      activity_details: {
        oauth_client_id: clientId,
        access_mode: "privacy_filtered_read_only",
        permitted_data_categories: [
          "foundations",
          "minimum_employee_identity",
          "due_diligence_metadata",
          "matter_admin_shell",
          "learning_operational_summary",
          "onboarding_aggregate_summary",
        ],
      },
    });

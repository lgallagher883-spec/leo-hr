...
export default function EmploymentFrameworkPage() {
  const [facts, setFacts] = useState<FoundationFact[]>([]);
  const [input, setInput] = useState("");
  const [loadingFacts, setLoadingFacts] = useState(true);
  const [sending, setSending] = useState(false);
  const [leoMessage, setLeoMessage] = useState(
    "I've reviewed what I currently understand about your employment arrangements. Is everything below still accurate, or has anything changed?"
  );
  const [errorMessage, setErrorMessage] = useState("");

  async function loadEmploymentFramework() {
    ...
    const { data, error } = await supabase
      ...
      .select("id, key, value")
      .eq("section", "Employment Framework")
      .order("created_at", { ascending: true });

    if (error) {
      console.error("Error loading employment framework:", error);

      setErrorMessage(
        "I couldn't load your Employment Framework just now. Please refresh the page and try again."
      );
  ...
  existingFacts: facts.map((fact) => ({
    ...
    value: fact.value,
      ...
      setLeoMessage(
        data.reply ||
          "Thank you. I've updated my understanding of your employment arrangements."
      );

      await loadEmploymentFramework();
    } catch (error) {
      console.error("Error updating employment framework:", error);

      setErrorMessage(
        "I couldn't update your Employment Framework just now. Please try again."
      );
    ...
    <div style={pageStyle}>
      <div style={headerStyle}>
        <div>
          ...
          <h1 style={titleStyle}>Employment Framework</h1>

          <p style={subtitleStyle}>
            Review and update Leo's understanding of your employment
            arrangements.
          </p>
        </div>
      ...
      <div style={conversationCardStyle}>
        ...
        <div style={factsSectionStyle}>
          ...
          {loadingFacts ? (
            <div style={emptyStateStyle}>
              I'm retrieving your Employment Framework...
            </div>
          ) : facts.length === 0 ? (
            <div style={emptyStateStyle}>
              I don't yet have any Employment Framework information recorded.
              Tell me about your organisation's employment arrangements below
              and I'll begin building it.
            </div>
          ) : (
        ...
        <div style={conversationPromptStyle}>
          ...
          <p style={promptTextStyle}>
            Tell Leo naturally. For example: “Our holiday year now runs from
            January to December and our probation period is six months.”
          </p>
          ...
          <div style={buttonRowStyle}>
            ...
            >
              {sending ? "Leo is updating..." : "Send to Leo"}
            </button>
          </div>
        </div>
      </div>

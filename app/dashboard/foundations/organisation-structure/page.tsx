...
export default function OrganisationStructurePage() {
  ...
  const [leoMessage, setLeoMessage] = useState(
    "I've reviewed what I currently understand about how your organisation is structured. Is everything below still accurate, or has anything changed?"
  );
  ...
  async function loadOrganisationStructure() {
    ...
    const { data, error } = await supabase
      ...
      .select("id, key, value")
      .eq("section", "Organisation Structure")
      .order("created_at", { ascending: true });

    if (error) {
      console.error("Error loading organisation structure:", error);

      setErrorMessage(
        "I couldn't load your Organisation Structure just now. Please refresh the page and try again."
      );
  ...
  async function sendUpdate() {
    ...
    try {
      ...
      setLeoMessage(
        data.reply ||
          "Thank you. I've updated my understanding of your organisation structure."
      );

      await loadOrganisationStructure();
    } catch (error) {
      console.error("Error updating organisation structure:", error);

      setErrorMessage(
        "I couldn't update your Organisation Structure just now. Please try again."
      );
    ...
    <div style={pageStyle}>
      <div style={headerStyle}>
        <div>
          ...
          <h1 style={titleStyle}>Organisation Structure</h1>

          <p style={subtitleStyle}>
            Review and update Leo's understanding of your teams, locations and
      ...
      <div style={conversationCardStyle}>
        ...
        <div style={factsSectionStyle}>
          ...
          {loadingFacts ? (
            <div style={emptyStateStyle}>
              I'm retrieving your Organisation Structure...
            </div>
          ) : facts.length === 0 ? (
            <div style={emptyStateStyle}>
              I don't yet have any Organisation Structure information recorded.
              Tell me about your locations, teams and managers below and I'll
              begin building it.
            </div>
          ) : (
        ...
        <div style={conversationPromptStyle}>
          ...
          <p style={promptTextStyle}>
            Tell Leo naturally. For example: “We now operate from three sites
            and each site has its own manager.”
          </p>

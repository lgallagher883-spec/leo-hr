...
const resourceTitle = "Employee Exit Checklist";
const resourceId = "employee-exit-checklist";
const resourceSummary =
  "A comprehensive checklist for managing employee departures consistently, securely and compliantly.";
...
export default function EmployeeExitChecklistPage() {
  ...
  function downloadWord() {
    ...
    anchor.href = url;
    anchor.download = "LEO-Employee-Exit-Checklist.doc";
...
<section><h2>How to use this checklist</h2><p>Use this checklist whenever an employee leaves the organisation, whether through resignation, dismissal, retirement, redundancy or the end of a fixed-term contract. Complete every applicable action before closing the employee record.</p></section>

<section><h2>Employee details</h2><div className="details-grid">
<div><strong>Employee</strong><span /></div>
<div><strong>Role</strong><span /></div>
<div><strong>Manager</strong><span /></div>
<div><strong>Leaving reason</strong><span /></div>
<div><strong>Leaving date</strong><span /></div>
<div><strong>Checklist owner</strong><span /></div>
</div></section>

{[
{title:"Before employment ends",items:[
"Confirm resignation, dismissal or other leaving reason in writing",
"Confirm notice period and final working day",
"Arrange handover plan",
"Identify business-critical work requiring transfer",
"Arrange exit meeting or interview where appropriate",
"Notify HR, payroll and relevant managers"
]},
{title:"Payroll and contractual matters",items:[
"Calculate final salary",
"Calculate outstanding annual leave",
"Confirm statutory or contractual payments",
"Process deductions authorised by law or contract",
"Issue final payslip",
"Provide any required employment documentation"
]},
{title:"Company property and access",items:[
"Recover keys, passes, equipment and devices",
"Recover company documents and records",
"Disable system and remote access on the agreed date",
"Remove email, software and third-party access",
"Transfer ownership of business files where required",
"Record any outstanding items"
]},
{title:"Compliance and records",items:[
"Update employee status in LEO",
"Archive employee record",
"Apply document retention periods",
"Update organisational charts where required",
"Close outstanding compliance actions",
"Record the departure in the audit trail"
]},
{title:"Knowledge and business continuity",items:[
"Complete handover",
"Notify customers or stakeholders where appropriate",
"Transfer responsibilities",
"Identify training needs for replacement or team",
"Record lessons learned if appropriate"
]},
{title:"Final review",items:[
...
].map((group)=>(<section key={group.title}><h2>{group.title}</h2><div className="checklist-table"><div className="checklist-header"><span>Done</span><span>Action</span><span>Owner</span><span>Date</span></div>{group.items.map((item)=><div className="checklist-row" key={item}><span className="check-box">☐</span><span>{item}</span><span /><span /></div>)}</div></section>))}
...
<Link className="related-link" href="/dashboard/policies/factsheets/ending-employment">Ending Employment Factsheet</Link>
<Link className="related-link" href="/dashboard/policies/letters">Letter Templates</Link>

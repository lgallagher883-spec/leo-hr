...
export default function EmployerSupportAskLeo({matterId,matter}:{matterId:number;matter:{title:string;description:string;status:string;matterType:string;subject:string}}){
 ...
 async function generateLeoReply(text:string,conversationMessages:Message[],appendUser:boolean){
  ...
  try{
   ...
   setMessages(current=>[...current,{role:"leo",content:""}]);streamedLeo=true;
   while(true){const {done,value}=await reader.read();buffer+=decoder.decode(value||new Uint8Array(),{stream:!done});const lines=buffer.split("\n");buffer=done?"":lines.pop()||"";for(const line of lines){if(!line.trim())continue;try{const event=JSON.parse(line);if(event.type==="delta"&&typeof event.delta==="string")leoText+=event.delta;if(event.type==="persisted"){leoPersisted=true;if(typeof event.messageId==="number")persistedMessageId=event.messageId;if(typeof event.createdAt==="string")persistedCreatedAt=event.createdAt;}if(event.type==="persist_error")throw new Error(event.error||"Leo response could not be saved.");if(event.type==="error")throw new Error(event.error||"Leo unavailable");}catch(parseError){if(parseError instanceof SyntaxError)continue;throw parseError;}}setMessages(current=>{const copy=[...current];copy[copy.length-1]={role:"leo",content:leoText};return copy});if(done)break;}
   if(buffer.trim()){try{const event=JSON.parse(buffer);if(event.type==="delta"&&typeof event.delta==="string")leoText+=event.delta;if(event.type==="persisted"){leoPersisted=true;if(typeof event.messageId==="number")persistedMessageId=event.messageId;if(typeof event.createdAt==="string")persistedCreatedAt=event.createdAt;}if(event.type==="persist_error")throw new Error(event.error||"Leo response could not be saved.");}catch(e){if(e instanceof SyntaxError){}else throw e;}}
   if(!leoText.trim())throw new Error("Leo returned an empty response.");
   if(!leoPersisted)throw new Error("Leo response could not be added to the Matter record.");if(persistedMessageId)setMessages(current=>{const copy=[...current];const last=copy[copy.length-1];if(last?.role==="leo")copy[copy.length-1]={...last,id:persistedMessageId,created_at:persistedCreatedAt};return copy});
  }catch(e){setMessages(current=>{const next=current.filter(m=>m.content.trim()!=="");if(streamedLeo&&!leoPersisted&&next[next.length-1]?.role==="leo")next.pop();return next});setError(e instanceof Error?e.message:"Leo could not complete that message. No changes have been made to the matter outside this conversation.");}

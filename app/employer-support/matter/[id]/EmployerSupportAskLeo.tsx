...
export default function EmployerSupportAskLeo({matterId,matter}:{matterId:number;matter:{title:string;description:string;status:string;matterType:string;subject:string}}){
 ...
 async function generateLeoReply(text:string,conversationMessages:Message[],appendUser:boolean){
  ...
  let streamedLeo=false;let leoPersisted=false;
  try{
   ...
   const leoSave=await fetch("/api/employer-support/matters/"+matterId+"/messages",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({role:"leo",content:leoText.trim()})});const savedLeo=await leoSave.json().catch(()=>({}));if(!leoSave.ok)throw new Error(savedLeo.error||"Leo response could not be saved.");leoPersisted=true;if(savedLeo.message?.id)setMessages(current=>{const copy=[...current];const last=copy[copy.length-1];if(last?.role==="leo")copy[copy.length-1]={...last,id:savedLeo.message.id,created_at:savedLeo.message.created_at};return copy});

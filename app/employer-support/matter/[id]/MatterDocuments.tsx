...
export default function MatterDocuments({matterId,documents}:{matterId:number;documents:Doc[]}){
 const [open,setOpen]=useState(false);const [openingId,setOpeningId]=useState<number|null>(null);const [error,setError]=useState("");const drafts=documents.filter(d=>d.document_type==="Leo draft"||d.status==="Draft");
 async function openDocument(documentId:number){setOpeningId(documentId);setError("");try{const r=await fetch(`/api/employer-support/matters/${matterId}/documents/${documentId}/open`);const p=await r.json().catch(()=>({}));if(!r.ok||!p.url)throw new Error(p.error||"The document could not be opened.");window.open(p.url,"_blank","noopener,noreferrer");}catch(e){setError(e instanceof Error?e.message:"The document could not be opened.");}finally{setOpeningId(null)}}

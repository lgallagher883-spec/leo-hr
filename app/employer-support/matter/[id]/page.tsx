...
export default async function EmployerSupportMatterPage({params}:{params:Promise<{id:string}>}){
 const {id}=await params;const matterId=Number(id);if(!Number.isSafeInteger(matterId)||matterId<=0)notFound();
 ...
 const [matterResult,actionsResult,documentsResult,timelineResult]=await Promise.all([
  ...
  (supabase as any).from("matter_documents").select("id,title,document_type,status,source,file_name,storage_path,created_at").eq("matter_id",matterId).order("created_at",{ascending:false}),
 ...
 if(matterError||actionsError||documentsError||timelineError)return <EmployerSupportShell><main className={styles.content}><Link href="/employer-support" className={styles.back}>← Back to your matters</Link><section className={styles.loadError}><span>!</span><div><h2>This matter could not be fully loaded</h2><p>Nothing has been changed. Refresh the page to try again. If the problem continues, use Help and support.</p></div></section></main></EmployerSupportShell>;
 if(!matter)notFound();
 const openActions=(actions??[]).filter((a:any)=>a.status==="open");
 const completed=String(matter.status).toLowerCase()==="completed";
 return <EmployerSupportShell><main className={styles.content}>
  ...
  <div className={styles.matterPulse}><div><span>{completed?"Matter status":"Next step"}</span><strong>{completed?"Matter concluded":openActions[0]?.title||"Continue with Leo"}</strong><small>{completed?"This matter is closed and its record is read-only.":openActions[0]?.detail||"Tell Leo what has changed, or ask what you should do next."}</small></div><div><span>Actions</span><strong>{openActions.length}</strong><small>{openActions.length===1?"open action":"open actions"}</small></div><div><span>Documents</span><strong>{documents?.length??0}</strong><small>in this matter</small></div></div>
  {!completed?<MatterTools matterId={matter.id}/>:null}<section className={styles.matterLayout}><div className={styles.matterMain}>
   <MatterActions matterId={matter.id} initialActions={actions??[]} closed={completed}/>
   <MatterDocuments matterId={matter.id} closed={completed} documents={(documents??[]).map((document:any)=>({id:document.id,title:document.title,document_type:document.document_type,status:document.status,source:document.source,file_name:document.file_name,created_at:document.created_at,hasFile:Boolean(document.storage_path)}))}/>

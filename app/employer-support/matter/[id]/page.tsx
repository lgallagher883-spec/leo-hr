...
export default async function EmployerSupportMatterPage({params}:{params:Promise<{id:string}>}){
 ...
 return <EmployerSupportShell><main className={styles.content}>
  ...
  {!completed?<MatterTools matterId={matter.id}/>:null}<section className={styles.matterLayout}><div className={styles.matterMain}>
   ...
   <MatterDocuments matterId={matter.id} closed={completed} documents={(documents??[]).map((document:any)=>({id:document.id,title:document.title,document_type:document.document_type,status:document.status,file_name:document.file_name,created_at:document.created_at,hasFile:Boolean(document.storage_path)}))}/>

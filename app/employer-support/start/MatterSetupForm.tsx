...
export default function MatterSetupForm(){
 ...
 return <div className={styles.setupForm}>
  ...
  {assessment?<section className={styles.assessmentExperience}>
    ...
    <div className={styles.journeySection}><p className={styles.eyebrow}>From issue to outcome</p><h3>A clear path, without having to work it out alone</h3><div className={styles.journeyTrack}><div><b>1</b><strong>Understand</strong><span>Leo helps you make sense of what has happened.</span></div><i>→</i><div><b>2</b><strong>Plan</strong><span>Know the right process and what needs doing next.</span></div><i>→</i><div><b>3</b><strong>Prepare</strong><span>Get ready for conversations, meetings and paperwork.</span></div><i>→</i><div><b>4</b><strong>Progress</strong><span>Keep the Matter moving with guidance at each stage.</span></div><i>→</i><div><b>5</b><strong>Conclude</strong><span>Reach and record a properly documented outcome.</span></div></div></div><div className={styles.purchasePanel}>
